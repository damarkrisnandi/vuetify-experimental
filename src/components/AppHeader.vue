<template>
    <v-app-bar :elevation="2" color="primary" density="compact">
        ...
        <!-- Breadcrumb -->
        <v-breadcrumbs v-if="breadcrumbs.length > 1" :items="breadcrumbs" class="d-none d-md-flex" color="white"
            divider="/">
            <template v-slot:item="{ item }">
                <v-breadcrumbs-item :to="item.to" :disabled="item.disabled" class="text-white">
                    {{ item.title }}
                </v-breadcrumbs-item>
            </template>
        </v-breadcrumbs>
        ...
        <v-menu>
            ...
            <v-list>
                ...
                <v-list-item prepend-icon="mdi-help-circle" title="Help" @click="() => { }"></v-list-item>
                <v-divider></v-divider>
                <v-list-item prepend-icon="mdi-information" title="About" @click="() => { }"></v-list-item>
            </v-list>
        </v-menu>
    </v-app-bar>
</template>
